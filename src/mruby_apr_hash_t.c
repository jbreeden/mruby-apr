...
mrb_APR_AprHashT_initialize(mrb_state* mrb, mrb_value self) {
  ...
  mruby_gift_apr_hash_t_data_ptr(self, native_object);
  return self;
}
#endif

mrb_value
mrb_APR_AprHashT_disown(mrb_state* mrb, mrb_value self) {
...
mrb_value
mrb_APR_AprHashT_belongs_to_ruby(mrb_state* mrb, mrb_value self) {
  mrb_value ruby_object;
  mrb_get_args(mrb, "o", &ruby_object);
...
void mrb_APR_AprHashT_init(mrb_state* mrb) {
  struct RClass* AprHashT_class = mrb_define_class_under(mrb, APR_module(mrb), "AprHashT", mrb->object_class);
...
#endif
  mrb_define_class_method(mrb, AprHashT_class, "disown", mrb_APR_AprHashT_disown, MRB_ARGS_ARG(1, 0));
  mrb_define_class_method(mrb, AprHashT_class, "belongs_to_ruby?", mrb_APR_AprHashT_belongs_to_ruby, MRB_ARGS_ARG(1, 0));
