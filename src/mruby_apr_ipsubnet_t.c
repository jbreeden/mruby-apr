...
mrb_value
mrb_APR_AprIpsubnetT_disown(mrb_state* mrb, mrb_value self) {
  mrb_value ruby_object;
  mrb_get_args(mrb, "o", &ruby_object);

  if (!mrb_obj_is_kind_of(mrb, ruby_object, mrb_class_ptr(self))) {
    mrb_raise(mrb, E_TYPE_ERROR, "APR::AprIpsubnetT.disown only accepts objects of type APR::AprIpsubnetT");
...
mrb_value
mrb_APR_AprIpsubnetT_belongs_to_ruby(mrb_state* mrb, mrb_value self) {
  mrb_value ruby_object;
  mrb_get_args(mrb, "o", &ruby_object);
...
#endif
  mrb_define_class_method(mrb, AprIpsubnetT_class, "disown", mrb_APR_AprIpsubnetT_disown, MRB_ARGS_ARG(1, 0));
  mrb_define_class_method(mrb, AprIpsubnetT_class, "belongs_to_ruby?", mrb_APR_AprIpsubnetT_belongs_to_ruby, MRB_ARGS_ARG(1, 0));
